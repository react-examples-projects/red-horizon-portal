...
axiosInstance.interceptors.request.use((req) => {
  const allowedUrls = ["/signup", "/login", "/reset-password", "/recover-password"];
  ...
  if ((req.url === "/auth/session" && !isValidToken()) || (!isAllowedUrl && !isValidToken())) {
...
axiosInstance.interceptors.response.use(
  ...
  (err) => {
    ...
    const isUrlsNotValid = config?.url !== "/session" && config?.url !== "/login";

    if ((isNotAuthorized && isUrlsNotValid) || (status === 403 && config?.url === "/session")) {
      removeToken();
    }
...
export const login = async (auth) => {
  const res = await axiosInstance.post("/login", auth);
  return res;
};